...
export class PumpFunSDK {
  ...
  async oneCreateAndBuy1(
    ...
    try {
      ...
      const _signature = await wallet.signTransaction(versionedTx)
      ...
      if (buyers.length > 0) {
        for (let i = 0; i < buyers.length; i++) {
          ...
          const _buyAmountSol = BigInt(Number(buyAmountSol2[i]) * 10 ** 9);
          const _buyAmount = globalAccount.getInitialBuyPrice(_buyAmountSol);
          const _buyAmountWithSlippage = calculateWithSlippageBuy(
            _buyAmount,
            slippageBasisPoints
          );
          const _buyTx = await this.getBuyInstructions(
            buyers[i].publicKey,
            mint.publicKey,
            globalAccount.feeRecipient,
            _buyAmount,
            _buyAmountWithSlippage,
          );
          const versionedTx = await addPriorityFees(this.connection, _buyTx, buyers[i].publicKey);
          buyTxs.push(versionedTx)
        }
      }

      // const result = await jitoWithAxios([createVersionedTx, ...buyTxs], wallet.publicKey);
      // if (result.confirmed)
